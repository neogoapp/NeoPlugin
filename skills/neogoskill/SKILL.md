---
name: neogoskill
description: Your gateway to NeoGo — run multi-agent workflows on your own worker (BYO Anthropic account) via natural language. Use whenever the user wants to run a NeoGo agent/flow, delegate a task, check a delegated task, install/connect their worker, or use a NeoGo utility (e.g. HTML→PDF). This skill is thin: it fetches how to operate from the server at runtime.
---
...
This skill is intentionally **thin**. The actual behavior — how to delegate (host) or how to execute (worker), the available agents/flows, and the utility skills — lives on the NeoGo server and is fetched on demand. That keeps the plugin tiny and always current.
...
Everything else — the delegation and execution protocols, agent/flow definitions, and utility skills — is served by the NeoGo MCP server (`get_playbook`, `get_skill`, and the tools they reference). This skill is just the entry point that connects you and routes you to the right behavior for your role.
